...
const statObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting && !entry.target.dataset.counted) {
      ...
      const nums = entry.target.querySelectorAll('.stat-num, .highlight-num');
      nums.forEach(num => {
        const text = num.textContent.trim();
        const isFloat = text.includes('.');
        const target = parseFloat(text);
        if (!isNaN(target)) {
          num.textContent = isFloat ? '0.0' : '0';
...
const countSections = document.querySelectorAll('.about-stats, .achievement-highlights');
countSections.forEach(section => statObserver.observe(section));
